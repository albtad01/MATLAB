...
function [Dati]=Eser03_dati(test) % function [...]= nome (input)

if strcmp(test, 'Es3') % sarà il primo es in tutta l'esecuzione
Dati = struct( 'name',             test,...
               ... % Test name, da non modificare
               'domain',           [-1,1],...       
               ... % Domain bounds, dominio che ci interessa     
               'mu',               0.25, ...
               ... % Diffusive term ..., cambio segno se problema diff.
               'beta',             0, ...
               ... % Advective term ..., applicazioni complesse
               'force',            '1-3*x.^2',...  
               ... % Forcing term, è la nostra f(x), non occorre linspace
               'exact_sol',        'x.^4-2*x.^2+1',...      
               ... % Definition of exact solution data dal testo
               'grad_exact',       ' 4*x.^3-4*x ',...    
               ... % Definition of exact gradient (x comp), derivata dell'exact sol
               'fem',              'P1',...         
               ... % P1-fe
               'nqn_1D',            2,...           
               ...
               'refinement_vector', [2:5],...   % 5=nRef ?
               ... % Refinement levels for  the error analysis, fatti i conti a mano per la seconda parte dell'Es1
               'visual_graph',      'Y',...
